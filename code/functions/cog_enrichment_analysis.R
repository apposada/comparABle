...
cog_enrichment_analysis <- function(x_modules, x_cog, specific_cogs = FALSE, fisher_pval = 0.05, plotting = FALSE, functional_categories = FALSE) {
  ...
  if (specific_cogs != FALSE) {
    ...
    cogs <- unique(sort(unlist(sapply(x_cog$cog, function(x){strsplit(x,"")})))) # must have numerics at the beginning
  }
  ...
  for (i in modules) {
    ...
    for (j in cogs) {
      ...
      contingency <- matrix(
        c(
          sum(x_module_cog$module == i & sapply(x_module_cog$cog, function(x){ any( strsplit(x,"") == j ) })),
          sum(x_module_cog$module == i & sapply(x_module_cog$cog, function(x){ any( strsplit(x,"") != j ) })),
          sum(x_module_cog$module != i & sapply(x_module_cog$cog, function(x){ any( strsplit(x,"") == j ) })),
          sum(x_module_cog$module != i & sapply(x_module_cog$cog, function(x){ any( strsplit(x,"") != j ) }))
        ),
