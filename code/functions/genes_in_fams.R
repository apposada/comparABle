key_genes_in_common_fams <- function(stats, ma, mb, ga, universe_a, gene2go_a, cog_a, universe, ...){ #IMPORTANT CAVEAT SEE BELOW
    ...
    for (i in 1:nrow(x)){ # for each instance in the stats tab; ie for each comparison
        ...
        fams_i_common <- unlist(strsplit(x$gfams_common[i], ",\ ")) # common fams of that comparison
...
key_genes_in_exclusive_fams <- function(
    stats, ga, gb, ma, mb, universe_a, universe_b, gene2go_a, gene2go_b, cog_a, b_cogs,
    ...){
    x <- stats
    ...
    #topgo
    x_fams_a_exclusive_GOs <- getGOs(x_fams_a_exclusive,gene_universe = universe_a, gene2GO = gene2go_a)
    
    x_fams_b_exclusive_GOs <- getGOs(x_fams_b_exclusive,gene_universe = universe_b, gene2GO = gene2go_b)
